...
void cmd_execute(int args_count, char *args[MAX_ARGS],
                 unsigned char **arena, int *size_arena);
...
void dump(unsigned char *arena, int *size_arena);
void alloc_block(unsigned char *arena, int *size_arena, int size_alloc);
void free_block(unsigned char* arena, int index);
void fill(unsigned char *arena, int index, int size, int value);
void show_arena(unsigned char *arena, int size_arena, int type);
FILE *fila;
int main() {
...
void initialize(int args_count, char *args[MAX_ARGS],
    ...
    *arena = (unsigned char*)calloc(n_bytes, sizeof(unsigned char));
...
void alloc_block(unsigned char *arena, int *size_arena, int size_alloc) {
    ...
    if (*size_arena - l_next - r_size >= size_alloc + 3 * sizeof(int)) {
...
void show_arena(unsigned char* arena, int size_arena, int type) {
    ...
    while (nxt) {
        ...
        // empty bytes -= bytes occupied by current block
        empty_bytes = empty_bytes - *((int*)(arena + cur + 2 * sizeof(int)));
        // aux3 = all bytes used by cur. block - admin. zone = only data bytes
    ...
    // print last occupied block
    if (type == 3) {
        printf("OCCUPIED %d bytes\n", *((int*)(arena + cur + 2 * sizeof(int))));
    }
    // if there is free space after last block update info
    if (size_arena - cur - *((int*)(arena + cur + 2 * sizeof(int))) > 0) {
        empty_blocks = empty_blocks + 1;
        if (type == 3) {
            // aux2 = size of free space
            int aux2;
            aux2 = size_arena - cur - *((int*)(arena + cur + 2 * sizeof(int)));
            printf("FREE %d bytes\n", aux2);
        }
    }
    // empty bytes -= total size occupied by last block
    empty_bytes -= *((int*)(arena + cur + 2 * sizeof(int)));
    // add size of data taken by last block to used bytes
    used_bytes += (*((int*)(arena + cur + 2 * sizeof(int))) - 3 * sizeof(int));

    // print number of free blocks & bytes
    if (type == 1) {
        printf("%d blocks (%d bytes) free\n", empty_blocks, empty_bytes);
    }
    // print number of used blocks & bytes, efficiency & fragmentation
    if (type == 2) {
        printf("%d blocks (%d bytes) used\n", n_blocks, used_bytes);

        // total number of used bytes = bytes for start index + admin. bytes
        // for all blocks + bytes used for data
        res_bytes = sizeof(int) + n_blocks * (3 * sizeof(int)) + used_bytes;
        // calculate efficiency using given formula
        double eff;  // efficiency
        eff = ((double)used_bytes) / res_bytes;
        eff = eff * 100;  // eff is a percentage
        printf("%d%% efficiency %d, %d\n", (int)eff, used_bytes, res_bytes);

        // calculate fragmentation using given formula
        double frag;
        frag = ((double)(empty_blocks - 1)) / n_blocks;
        frag = frag * 100;  // frag is a percentage
